...
CREATE TABLE sources (
    ...
    license             VARCHAR,
    target_metric       VARCHAR,       -- which metric/table this source feeds, e.g.
                                       -- 'bdd', 'ecc', 'tcc', 'isds', 'capacity_indicators',
                                       -- 'doctrinal_scores', 'subnational_overlays' —
                                       -- so the registry itself explains why a row exists
    verified            BOOLEAN NOT NULL DEFAULT FALSE,  -- FALSE = URL/access details
                                                          -- not yet confirmed; do not
